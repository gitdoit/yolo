"""
07_traffic_congestion_seg.py — 语义分割动态 ROI 交通拥堵检测

用 SegFormer(Cityscapes) 自动识别道路像素作为动态 ROI，
用 YOLOv8 检测车辆，计算车辆在道路区域的面积占比判定拥堵。
"""
...
# 视频流地址
STREAM_URL = (
    "http://10.100.121.12:8086/rtp/34020000001180001024_34020000001310002402.live.flv"
)
...
# 每隔多少帧更新一次道路分割掩码（分割较重，建议 30~60）
SEG_EVERY_N_FRAMES = 30
...
def main():
    ...
    while True:
        ...
        # 道路分割（每 N 帧更新）
        if road_mask is None or frame_idx % SEG_EVERY_N_FRAMES == 0:
            road_mask = segment_road(frame, seg_processor, seg_model, device)
